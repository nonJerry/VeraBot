...
from utility import date_from_txt

data = Core()
bot, local = data.bot, data.local
...
async def detect_image_text(img_url):
    ...
    text = await bot.loop.run_in_executor(None, img_to_txt, img)

    # get inverted text (run as coroutine to not block the event loop)
    inverted_text = await bot.loop.run_in_executor(None, img_to_txt, inverted_img)
